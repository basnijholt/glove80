...
- **Vertical stack** (`src/glove80/tui/screens/editor.py`):
  ...
  - **InspectorOverlay** (`src/glove80/tui/widgets/inspector.py#InspectorOverlay`) is a right-aligned slide-over mounted on a dedicated `overlay` CSS layer (`Screen { layers: base overlay; }`). It reuses `InspectorPanel`, so opening it never compresses the `KeyCanvas`. The overlay is hidden by default, appears when the ribbon toggle emits `InspectorToggleRequested`, focuses its `KeyInspector` when `InspectorFocusRequested` fires, and binds `Esc` to close + return focus to the widget that opened it (falling back to the canvas). Behavior is exercised by `tests/tui/integration/test_editor_rendering.py::test_inspector_overlay_toggles_and_resets_focus` and `tests/tui/integration/test_key_canvas.py::test_key_canvas_moves_selection_and_focuses_inspector`, and the visual reference lives in `docs/screenshots/inspector-overlay/{before,after}.txt` (captured at 140×40 via `scripts/tui_snapshot_demo.py`). The legacy drawer stays collapsed for compatibility until downstream UIs migrate.
  - **HoldTap tab scrolling** – Each inspector studio stays inside a `VerticalScroll (classes="inspector-scroll")`. HoldTap now mirrors Macros/Combos, so its list + detail form scroll cleanly (no double scrollbars) in both the drawer and InspectorOverlay when content exceeds the viewport.
  - **HRM Preview** – When `BuilderBridge` assembles HRM components it normalizes any non-TailorKey variant (e.g., `families/default` `factory_default`) to the TailorKey “windows” baseline before calling `build_hrm_layers`, while native TailorKey variants pass straight through; see `src/glove80/tui/services/builder_bridge.py`. `tests/tui/integration/test_features_tab.py` covers the behavior, and we ignore the repo-wide coverage gate for this flow.
